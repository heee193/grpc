...
class DebugStringBuilder {
 public:
  ...
  // Redact the value if it is not allow listed.
  void AddIfAllowListed(absl::string_view key, absl::string_view value);
 ...
 public:
  ...
  std::string DebugString() const {
    ...
    Log([&builder](absl::string_view key, absl::string_view value) {
      builder.AddIfAllowListed(key, value);
    });
    return builder.TakeOutput();
  }

  // This may log sensitive information.
  // Don't submit this into staging/production code.
  std::string DebugStringUnredactedDoNotSubmit() const {
    metadata_detail::DebugStringBuilder builder;
    Log([&builder](absl::string_view key, absl::string_view value) {
